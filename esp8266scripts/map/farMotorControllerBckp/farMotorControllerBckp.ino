...
void runMotor(const char * payload, size_t length) {
  Serial.println("refreshing \n");
  val = 1;
  
  //rainbow(10);
}
void stopMotor(const char * payload, size_t length) {
  Serial.println("refreshing \n");
  val = 0;
...
void setup() {
    ...
    webSocket.on("startMotor", runMotor);
    webSocket.on("stopMotor", stopMotor);
...
void loop() {
    ...
    //int newVal = digitalRead(inPin);
     // read input value
    if (val == 1 && canRotateMotors) {         // check if the input is HIGH (button released)
